...
async function generateDashboard() {
    ...
    const lookbackTime = now - lookbackMs;
    const sortedTelemetry = loopTelemetry
        .filter((entry) => !!entry.timestamp && !Number.isNaN(Date.parse(String(entry.timestamp))))
        .sort((a, b) => Date.parse(String(a.timestamp)) - Date.parse(String(b.timestamp)));
    const historicalPoint = [...sortedTelemetry].reverse().find((entry) => Date.parse(String(entry.timestamp)) <= lookbackTime)
        ?? sortedTelemetry[0]
        ?? null;

...
export default function ChatbotBuilder() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (newBot) => {
      ...
      try {
        // Validate knowledge sources exist as entities
        const validatedSources = [];
        for (const source of formData.knowledge_sources) {
          const entityName = source.entity_name || source.source_id || source.name;
          if (entityName) {
            try {
              // Try to fetch the entity to validate it exists
              await base44.entities[entityName]?.list?.({ limit: 1 });
              validatedSources.push({
                entity_name: entityName,
                allowed_operations: ['read']
              });
            } catch (error) {
              console.warn(`Entity ${entityName} not found, skipping knowledge source`);
            }
          }
        }

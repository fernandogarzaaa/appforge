...
export default function ChatOpsInterface() {
  ...
  const acknowledgeMutation = useMutation({
    mutationFn: (alertId) => base44.entities.AnomalyAlert.update(alertId, { status: 'acknowledged' }),
    onSuccess: () => {
      toast.success('Alert acknowledged');
      // Refetch active alerts
  ...
  const handleCommand = async () => {
    ...
    if (command === '/status') {
      ...
      if (alertId) {
        ...
        response = 'Please provide an alert ID: /acknowledge <id>';
      }

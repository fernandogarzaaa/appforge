...
function evaluateCondition(condition, variables) {
  ...
  // Only allow safe comparison and logical operators
  if (!/^[\d\s+\-*/().=!<>&&||truefalsenull\s]+$/.test(expr)) {
    throw new Error('Condition contains invalid characters');
  }

  try {
    // Use Function constructor with strict limitations for boolean evaluation

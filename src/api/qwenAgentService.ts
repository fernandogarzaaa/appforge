...
import { chimera, type ChatMessage } from './chimeraClient';
...
export interface AgentConfig {
  mode: 'react' | 'function_calling' | 'group_chat' | 'browser' | 'code_interpreter' | 'chat';
  systemPrompt?: string;
...
export class QwenAgentService {
  private chimera: typeof chimera;
   ...
   */
  async initialize(config: AgentConfig): Promise<boolean> {
   ...
   */
  async reactAgent(task: string, options?: {
    maxIterations?: number;
    tools?: ToolDefinition[];
  }): Promise<AgentResponse> {
   ...
   */
  async functionCalling(userMessage: string, tools: ToolDefinition[]): Promise<AgentResponse> {
    const systemMessage: ChatMessage = {
      role: 'system',
   ...
   */
  async groupChat(task: string, agentConfigs: {
    name: string;
    description: string;
    systemPrompt: string;
  }[]): Promise<AgentResponse> {
   ...
   */
  private parseToolCalls(content: string, tools: ToolDefinition[]): ToolCall[] {

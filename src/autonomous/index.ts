...
interface AutonomousSystemOptions {
  config?: Partial<AutonomousConfig>;
  autoStart?: boolean;
  onDecision?: (event: SystemEvent) => void;
  onHealthChange?: (status: AutonomousState['healthStatus']) => void;
}
...
export class AutonomousSystem extends EventEmitter {
  ...
  private improvementLoop: SelfImprovementLoop;
  private decisionTree: AutonomousDecisionTree;
  ...
  constructor(options: AutonomousSystemOptions = {}) {
    super();
   ...
   */
  private registerAppForgeComponents(): void {
   ...
   */
  private performPassiveModeActions(): void {
   ...
   */
  private async performActiveModeActions(): Promise<void> {
   ...
   */
  private async performAggressiveModeActions(): Promise<void> {
    ...
    if (systemHealth.status !== 'healthy') {
      ...
      await this.decisionTree.makeDecision(
  ...
  async createGoal(
    name: string,
    description: string,
    priority: AutonomousConfig['humanApprovalRequired'] extends string[] ? any : any = 'medium'
  ): Promise<Goal> {
   ...
   */
  private async handleHealthDegradation(metric: HealthMetric): Promise<void> {
  ...
  async createStrategicPlan(
    name: string,
    description: string,
    objectives: { description: string; priority: any; targetDate: Date; successCriteria: string[] }[]
  ): Promise<StrategicPlan> {
    const endDate = new Date();
    endDate.setMonth(endDate.getMonth() + 3); // 3 month default

    return this.decisionTree.createStrategicPlan(
      name,
      description,
      objectives,
      {
  ...
  async makeDecision(
    description: string,
    actions: { name: string; description: string; execute: () => Promise<unknown> }[],
    context: { goal: string; urgency?: number; constraints?: string[] }
  ): Promise<unknown> {
    ...
    const outcome = await this.decisionTree.makeDecision(
      description,
      fullActions,
      {
        goal: context.goal,
   ...
   */
  setMode(mode: AutonomousConfig['mode']): void {
  ...
  getStats(): {
    ...
    execution: ReturnType<ExecutionEngine['getStats']>;
    healing: ReturnType<SelfHealingSystem['getStats']>;
    improvement: ReturnType<SelfImprovementLoop['getStats']>;
    decision: ReturnType<AutonomousDecisionTree['getStats']>;
  } {
    return {
      state: this.getState(),
...
export function getAutonomousSystem(options?: AutonomousSystemOptions): AutonomousSystem {
...
export function resetAutonomousSystem(): void {
  if (globalAutonomousSystem) {

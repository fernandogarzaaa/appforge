...
function toggle(current: boolean | undefined, seedValue: number): boolean {
  if (seedValue > 0.7) return !(current ?? false);
  return current ?? false;
}

export function mutateGenome(genome: StrategyGenome, intensity: number, seed: number): StrategyGenome {
...
export function crossover(genomeA: StrategyGenome, genomeB: StrategyGenome, seed: number): StrategyGenome {

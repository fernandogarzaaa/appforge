...
function scoreGenome(genome, benchmarkIds) {
...
async function savePopulation(population) {
...
export async function runStrategySearch(benchmarkIds, seed, plateaued, previousBest) {
    ...
    const evaluated = basePopulation.map((genome) => {
    ...
    const nextPopulation = survivors.map((candidate) => candidate.genome);

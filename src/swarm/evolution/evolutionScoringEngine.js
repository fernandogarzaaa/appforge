export const DEFAULT_SCORE_GATE_CONFIG = {
...
export function calculateEvolutionScore(input) {
    ...
    const finalScore = Number((weightedBenchmarkScore * 0.7 +
        efficiencyScore * 0.15 +
        stabilityScore * 0.15).toFixed(4));
    return {
        finalScore,
...
export function compareScores(candidate, baseline, config = DEFAULT_SCORE_GATE_CONFIG) {
    if (!baseline) {
        return {
            improved: true,
            minDeltaMet: true,
    ...
    const regressionFailures = Object.entries(candidate.benchmarkAggregate)
        .filter(([name, score]) => {
        const baselineScore = baseline.benchmarkAggregate[name] ?? 0;

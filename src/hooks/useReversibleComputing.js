...
import { createReversibleState, createStateHistory, recordSnapshot, rollbackTo, getTimeline, clearHistory, getSnapshotCount, getReversibleStateData } from '@/lib/ReversibleComputing';
/**
 ...
 */
export function useReversibleComputing(initialSize = 8, maxSnapshots = 100) {
    ...
    const recordCurrentSnapshot = useCallback(async (description = '') => {
        if (!isReady || !stateRef.current || !historyRef.current)
            return;
        try {
    ...
    const jumpToSnapshot = useCallback(async (index) => {
        if (!isReady || !historyRef.current || index < 0 || index >= snapshots.length)
            return;
        try {

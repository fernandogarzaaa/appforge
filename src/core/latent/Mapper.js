...
class LatentMapper {
    ...
    analyzeSafety(content) {
        ...
        let score = 0.5;
        triggers.forEach(t => { if (content.includes(t))
            score += 0.05; });
    ...
    analyzeEfficiency(content) {
        ...
        if (content.includes('Promise.all'))
            score += 0.1;
        if (content.includes('memo') || content.includes('cache'))
            score += 0.1;
    ...
    analyzePurpose(filePath) {
        if (filePath.includes('.rs'))
            return 0.9; // Kernel/Governance
        if (filePath.includes('.ts'))
            return 0.6; // Business Logic

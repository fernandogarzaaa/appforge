...
// Moved outside Dashboard to prevent re-renders
const HeroSection = React.memo(({ ideaInput, setIdeaInput }) => (
  <motion.div
...
export default function Dashboard() {
  ...
  const totalStats = useMemo(() => projects.reduce(
    (acc, p) => ({
  ...
  const capabilities = useMemo(() => [
    ...
    ...(user?.role === 'admin' ? [
      { icon: Settings, label: 'Admin Panel', href: createPageUrl('AdminDashboard'), color: 'text-red-700 bg-red-100' }
    ] : [])
  ], [user]);

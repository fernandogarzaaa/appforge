 ...
 */
export const ddosProtectionMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const clientIP = (req.headers['x-forwarded-for'] as string) || req.socket.remoteAddress || 'unknown';
...
export function setupSecurityMiddleware(app: any) {
  ...
  app.get('/api/security/status', (req: Request, res: Response) => {
    ...
    const rateLimitStats = rateLimiter.getStatus(
  ...
  // Analytics endpoint
  app.get('/api/security/analytics', async (req: Request, res: Response) => {

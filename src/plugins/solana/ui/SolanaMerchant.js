...
const SolanaMerchant = ({ onPaymentSuccess, amount = 0.05, label = 'AppForge Merchant' }) => {
    ...
    return (_jsxs("div", { className: "flex flex-col items-center gap-4 text-slate-300", children: [_jsxs("div", { className: "w-full bg-slate-800/50 rounded flex flex-col items-center justify-center border border-slate-700 overflow-hidden p-4", children: [auditStatus && (_jsx("div", { className: `w - full mb - 3 p - 2 rounded text - [10px] font - mono font - bold border ${auditStatus.includes('BLOCKED') ? 'bg-red-500/20 border-red-500 text-red-400' : 'bg-emerald-500/20 border-emerald-500 text-emerald-400'} `, children: auditStatus })), qrCode ? (_jsxs("div", { className: "text-center w-full", children: [_jsx("div", { className: "bg-white p-2 rounded mb-4 overflow-hidden", children: _jsx("div", { className: "text-slate-900 text-[8px] break-all mono leading-tight", children: qrCode }) }), _jsxs("span", { className: "text-blue-400 font-bold", children: [amount.toFixed(2), " SOL"] })] })) : (_jsxs("div", { className: "flex flex-col items-center gap-2", children: [_jsx("span", { className: "text-slate-500 text-[10px] uppercase font-bold", children: "Secure Checkout" }), _jsxs("span", { className: "text-2xl font-bold text-white", children: [amount, " SOL"] })] }))] }), _jsxs("div", { className: "flex gap-2 w-full", children: [_jsx("button", { onClick: generatePayment, className: "flex-1 py-2 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded text-[10px] uppercase tracking-widest transition-all", children: "QR Pay" }), _jsx("button", { onClick: handleDirectPay, disabled: !isOracleOnline, className: `flex - 1 py - 2 font - bold rounded text - [10px] uppercase tracking - widest transition - all ${isOracleOnline ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-slate-800 text-slate-600 cursor-not-allowed'} `, children: isOracleOnline ? 'Iron Pay (Web3)' : 'Oracle Offline' })] }), _jsxs("div", { className: "grid grid-cols-2 gap-2 w-full mt-2 border-t border-slate-700/50 pt-2", children: [_jsx("button", { onClick: async () => {
                            ...
                            if (result.risk_score > 0.7)
                                setStatus('Thwarted!');
                        }, className: "py-1 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/30 rounded text-[9px] uppercase font-mono", children: "Simulate Attack" }), _jsx("button", { onClick: async () => {
                        ...
                        }, className: "py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 border border-emerald-500/30 rounded text-[9px] uppercase font-mono opacity-50 cursor-not-allowed", title: "Requires Real Wallet Signature", children: "Simulate Safe (Wallet)" })] }), _jsxs("div", { className: "text-[10px] font-mono text-slate-500 uppercase tracking-widest", children: ["Status: ", _jsx("span", { className: status.includes('Complete') ? 'text-emerald-400' : status.includes('Blocked') ? 'text-red-400' : 'text-slate-400', children: status })] })] }));
};

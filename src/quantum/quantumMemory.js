 ...
 */
import { complexFromPolar, complexMagnitudeSq, complexMultiply, complexConjugate, } from './types';
/**
...
export class QuantumMemory {
    ...
    referenceBeam;
    rng;
    constructor(config, maxCapacity = 1024, embeddingDim = 128) {
        this.config = config;
     ...
     */
    search(queryEmbedding, topK = 5, coherenceFloor = 0.5) {
        ...
        const amplitudes = new Array(n).fill({ real: 1 / Math.sqrt(n), imaginary: 0 });
     ...
     */
    consolidate(similarityThreshold = 0.9) {
        let mergeCount = 0;
        for (let i = 0; i < this.cells.length; i++) {
            for (let j = i + 1; j < this.cells.length; j++) {
                const sim = this.cosineSimilarity(this.cells[i].entry.embedding, this.cells[j].entry.embedding);
    ...
    encode(embedding) {
        ...
        for (let i = 0; i < this.embeddingDim; i++) {
            const signal = complexFromPolar(Math.abs(embedding[i] ?? 0), (embedding[i] ?? 0) >= 0 ? 0 : Math.PI);
    ...
    measureFidelity(pattern, original) {
     ...
     */
    groverIteration(amplitudes, oracleScores) {
    ...
    computeOracleScores(queryEmbedding) {
    ...
    measureSearchCoherence(amplitudes, oracleScores) {
        ...
        if (highScoreIndices.length > 1) {
            const phases = highScoreIndices.map((i) => Math.atan2(amplitudes[i].imaginary, amplitudes[i].real));

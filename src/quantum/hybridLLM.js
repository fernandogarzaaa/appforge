...
export class HybridLLM {
    ...
    constructor(config, classical, options) {
        this.config = config;
        this.classical = classical;
        this.attention = new QuantumAttention(config, options?.numAttentionHeads ?? 8, options?.attentionHeadDim ?? 64);
        this.decoder = new QuantumDecoder(config);
        this.memory = new QuantumMemory(config, options?.memoryCapacity ?? 1024, options?.embeddingDim ?? 128);
    }
    /**
    ...
    consolidateMemory(similarityThreshold) {
    ...
    async processClassical(task, memoryContext, startTime) {
    ...
    async processQuantum(task, memoryContext, startTime) {
        const contextStrings = memoryContext?.entries.map((e) => e.entry.content) ?? [];
        ...
        const decoded = await this.decoder.decodeSequence(async (ctx) => {
    ...
    async processHybrid(task, memoryContext, routing, startTime) {
        const contextStrings = memoryContext?.entries.map((e) => e.entry.content) ?? [];
        // Run both pipelines
    ...
    classicalCoherence(output, task) {

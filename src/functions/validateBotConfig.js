...
function validateTriggerConfig(trigger) {
    ...
    switch (type) {
        case 'schedule':
            if (!config.frequency)
                warnings.push('Schedule frequency not specified');
            break;
        case 'webhook':
        case 'api_endpoint':
...
function generateSuggestions(botConfig) {
    ...
    // Check for logging
    const hasLogging = botConfig.nodes?.some(n => n.config?.actionType === 'log' || n.config?.details?.includes('log'));
    if (!hasLogging) {
        suggestions.push('Consider adding logging nodes for debugging');
    }
    // Check trigger appropriateness
    if (botConfig.trigger?.type === 'schedule' && botConfig.nodes?.some(n => n.config?.actionType === 'send_email')) {

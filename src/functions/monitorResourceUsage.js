    ...
    try {
        ...
        try {
            ...
            for (const automation of automations) {
                if (automation.is_active) {
                    ...
                    const logs = await base44.entities.AuditLog.filter({ changes: { contains: automation.id } }, '-created_date', 50).catch(() => []);
        ...
        try {
            ...
            for (const workflow of workflows) {
                const executions = await base44.entities.WorkflowExecution.filter({ workflow_id: workflow.id }, '-created_date', 50).catch(() => []);
                ...
                const estimatedMemoryMB = 30 + ((Array.isArray(executions) ? executions.length : 0) * 1.5);
        ...
        const allResources = [
            ...Object.values(metrics.automations),
            ...Object.values(metrics.workflows)
        ];

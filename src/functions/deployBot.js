...
Deno.serve(async (req) => {
    try {
        ...
        if (!validation.valid) {
            return Response.json({ error: 'Invalid bot configuration', details: validation.errors }, { status: 400 });
        }
        ...
        if (!triggerSetup.success) {
            return Response.json({ error: 'Failed to set up trigger', details: triggerSetup.error }, { status: 500 });
        }

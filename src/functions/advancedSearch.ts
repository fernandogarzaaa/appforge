...
const normalizeDoc = (item: Record<string, any>, type: string) => {
  const name =
    item.name ||
    item.title ||
    item.label ||
...
const scoreDoc = (doc: any, query: string) => {
  if (!query) return 0;
  const q = query.toLowerCase();
  const name = String(doc.name || '').toLowerCase();
  const description = String(doc.description || '').toLowerCase();
  const tags = (doc.tags || []).map((t: string) => String(t).toLowerCase());

  let score = 0;
  if (name === q) score += 100;
  if (name.includes(q)) score += 70;
  if (description.includes(q)) score += 30;
  if (tags.some((tag: string) => tag.includes(q))) score += 20;
...
const matchesQuery = (doc: any, query: string) => {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const safeList = async (entityName: string) => {

...
Deno.serve(async (req) => {
    try {
        ...
        if (!buildWebhook) {
            return Response.json({ error: 'Mobile build provider not configured. Set MOBILE_BUILD_WEBHOOK_URL.' }, { status: 501 });
        }
        ...
        try {
            ...
            return Response.json({ error: 'Failed to queue mobile build request' }, { status: 502 });
        }

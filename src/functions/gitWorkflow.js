...
const githubRequest = async (token, method, endpoint, body) => {
...
${llm?.content || llm?.response || llm}
      `.trim();
        ...
        if (action === 'generate_changelog') {
            ...
            const compare = await githubRequest(githubToken, 'GET', `/repos/${owner}/${repo}/compare/${baseVersion}...main`);
            const commits = (compare.commits || []).map((commit) => commit.commit?.message || commit.sha);

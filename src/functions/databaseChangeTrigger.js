...
Deno.serve(async (req) => {
    try {
        ...
        if (!entityName || !eventType) {
            return Response.json({ error: 'Missing entityName or eventType' }, { status: 400 });
        }
        ...
        const matchingBots = bots.filter(bot => {
            ...
            if (config.entity !== entityName)
                return false;
            // Check event type
            if (config.event !== 'all' && config.event !== eventType)
                return false;
            // Check conditions if specified
            if (config.conditions && eventType === 'update') {
                if (!evaluateConditions(config.conditions, newData))
                    return false;
            }
            return true;
        });
...
async function getLastExecution(botId, base44) {
    try {
        const executions = await base44.entities.WorkflowExecution.filter({ bot_id: botId }, '-created_date', 1);

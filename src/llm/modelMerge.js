 ...
 */
function matchLayerGlob(pattern, layerName) {
...
class SafetensorsLoader {
    ...
    async loadFromLocal(model) {
        ...
        const weightFiles = (await fs.readdir(model.path)).filter((f) => f.endsWith('.safetensors') || f.endsWith('.bin'));
    ...
    async saveWeights(weights, outputPath, dtype = 'float16') {
        const fs = await import('fs/promises');
        const path = await import('path');
...
export class ModelMerger {
    ...
    async merge(config, onProgress) {
        ...
        try {
            ...
            for (const mw of modelWeights) {
                ...
                if (!compat.compatible) {
                    ...
                    if (compat.blocking) {
                        return {
                            ok: false,
                            error: new Error(`Architecture mismatch: ${compat.warnings.join('; ')}`),
                        };
                    }
                }
            }
            ...
            const outputPath = await this.loader.saveWeights(mergedWeights, config.outputDir, config.outputDtype);
     ...
     */
    async mergeSlerp(config, baseWeights, modelWeights, onProgress) {
        ...
        for (let li = 0; li < layerNames.length; li++) {
            ...
            merged.set(layerName, {
                ...
                dtype: config.outputDtype ?? baseLayer.dtype,
                data: new Float32Array(Array.from(currentData)),
            });
            if (li % 10 === 0) {
                onProgress?.({
                    stage: 'merging',
     ...
     */
    async mergeTaskArithmetic(config, baseWeights, modelWeights, onProgress) {
        ...
        for (let li = 0; li < layerNames.length; li++) {
            ...
            for (const mw of modelWeights) {
                ...
                const taskVector = subVectors(new Float32Array(modelLayer.data), new Float32Array(baseLayer.data));
            ...
            merged.set(layerName, {
                ...
                shape: [...baseLayer.shape],
                dtype: config.outputDtype ?? baseLayer.dtype,
                data: new Float32Array(Array.from(resultData)),
            });
            if (li % 10 === 0) {
                onProgress?.({
                    stage: 'merging',
     ...
     */
    async mergeModelSoup(config, baseWeights, modelWeights, onProgress) {
        this.logger.info('Performing Model Soup merge');
        const merged = new Map();
        const layerNames = Array.from(baseWeights.weights.keys());
        // Include base model in the soup
        const allModels = [
            { entry: { model: config.baseModel, weight: 1.0 }, weights: baseWeights },
            ...modelWeights,
        ];
        for (let li = 0; li < layerNames.length; li++) {
            const layerName = layerNames[li];
            ...
            merged.set(layerName, {
                ...
                dtype: config.outputDtype ?? refLayer.dtype,
                data: new Float32Array(Array.from(resultData)),
            });
            if (li % 10 === 0) {
                onProgress?.({
     ...
     */
    async mergeTIES(config, baseWeights, modelWeights, onProgress) {
        ...
        for (let li = 0; li < layerNames.length; li++) {
            ...
            const trimmedVectors = taskVectors.map((tv) => {
                ...
                for (let i = 0; i < size; i++)
                    magnitudes[i] = Math.abs(tv.data[i]);
     ...
     */
    async mergeDARE(config, baseWeights, modelWeights, onProgress) {
    ...
    checkArchitectureCompatibility(base, model) {
        const warnings = [];
        let blocking = false;
        if (base.config.hiddenSize !== model.config.hiddenSize) {
            warnings.push(`Hidden size mismatch: ${base.config.hiddenSize} vs ${model.config.hiddenSize}`);
            blocking = true;
        }
        if (base.config.numHiddenLayers !== model.config.numHiddenLayers) {
            warnings.push(`Layer count mismatch: ${base.config.numHiddenLayers} vs ${model.config.numHiddenLayers}`);
            blocking = true;
        }
        if (base.config.vocabSize !== model.config.vocabSize) {
            warnings.push(`Vocab size mismatch: ${base.config.vocabSize} vs ${model.config.vocabSize}`);
            // Non-blocking – can handle with embedding resize
        }
        if (base.config.numAttentionHeads !== model.config.numAttentionHeads) {
            warnings.push(`Attention heads mismatch: ${base.config.numAttentionHeads} vs ${model.config.numAttentionHeads}`);
            blocking = true;
        }
        return {
     ...
     */
    static presetHermesZephyrSlerp(outputDir) {
        return {
     ...
     */
    static presetFullSoup(outputDir) {
        return {
     ...
     */
    static presetTaskArithmeticCoding(outputDir) {
        return {

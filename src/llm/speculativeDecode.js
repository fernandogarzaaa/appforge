...
export class SpeculativeDecoder {
     ...
     */
    async trainMedusaHeads(trainingData) {
...
`;
     ...
     */
    async initializeRESTDatastore(texts) {
    ...
    async generateREST(prompt) {
        ...
        while (generated.length < this.config.maxTokens) {
            ...
            if (generated.length > this.config.maxTokens)
                break;
        }
        const generationTime = Date.now() - startTime;
        return {
    ...
    verifyCandidates(context, candidates) {
     ...
     */
    async generateLookahead(prompt) {
        const startTime = Date.now();

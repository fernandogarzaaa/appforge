 ...
 * @returns {Array<Array<ComplexNumber>>} Density Matrix
 */
export function stateToDensityMatrix(state) {
  if (state.numQubits > 10) {
    throw new Error('Density Matrix simulation limited to 10 qubits for performance.');
  }
  const size = state.amplitudes.length;
  const matrix = Array(size).fill(null).map(() => Array(size));

  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      // |i><j| element = amp[i] * conj(amp[j])
 ...
 * @param {Array<Array<ComplexNumber>>} rho
 * @param {number} probability - Noise probability (0-1)
 * @returns {Array<Array<ComplexNumber>>} New Density Matrix
 */
export function applyDepolarizingNoise(rho, probability) {
  const size = rho.length;
  const identityPart = probability / size;
  const originalPart = 1 - probability;

  // rho_new = (1-p) * rho + (p/d) * I
  return rho.map((row, i) =>
    row.map((val, j) => {
      let newVal = multiplyComplex(val, complex(originalPart, 0));
      if (i === j) {
        newVal = addComplex(newVal, complex(identityPart, 0));
      }
      return newVal;
    })
  );

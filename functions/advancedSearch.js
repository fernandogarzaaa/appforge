    ...
    try {
        ...
        switch (action) {
            case 'search': {
                ...
                if (sortBy === 'date') {
                    sortedResults.sort((a, b) => (b.metadata?.updated_date || 0) - (a.metadata?.updated_date || 0));
                }
                else if (sortBy === 'type') {
 ...
 */
async function performSearch(base44, query, filters, userId) {
    const results = [];
    ...
    if (filters) {
        return results.filter(r => {
            if (filters.type && r.type !== filters.type)
                return false;
            if (filters.projectId && r.metadata?.project_id !== filters.projectId)
                return false;
            return true;
        });
    }
 ...
 */
function extractHighlights(query, text, maxHighlights = 3) {
    if (!text)
        return [];
    const highlights = [];
    const queryLower = query.toLowerCase();
 ...
 */
async function generateAutocomplete(base44, query, userId) {
    const suggestions = [];
 ...
 */
async function searchCode(base44, query, filters, userId) {
 ...
 */
async function searchFiles(base44, query, filters, userId) {

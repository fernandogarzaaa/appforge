...
async function verifyPaymongoSignature(body: string, signature: string | null, webhookSecret: string): Promise<boolean> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isValid = await verifyPaymongoSignature(body, signature, webhookToken);
    if (!isValid) {
      logger.warn('PayMongo webhook signature verification failed');

...
function buildSuggestedSplits(content: string, maxModuleLines: number) {
...
Deno.serve(async (req) => {
  try {
    ...
    const payload = await req.json() as BlueprintRequest;
    ...
    const content = await (Deno as any).readTextFile(targetPath);

...
Deno.serve(async (req) => {
    try {
        ...
        if (!validation.valid) {
            return Response.json({ success: false, error: 'Invalid bot configuration', details: validation.errors }, { status: 400 });
        }
        ...
        if (!triggerSetup.success) {
            return Response.json({ success: false, error: 'Failed to set up trigger', details: triggerSetup.error }, { status: 500 });
        }
 ...
 */
async function setupWebhookTrigger(bot, _base44) {
    try {
 ...
 */
async function setupEmailTrigger(bot, _base44) {
    try {
        const config = bot.trigger?.config || {};
        return {
            success: true,

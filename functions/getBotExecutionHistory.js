...
Deno.serve(async (req) => {
    try {
        ...
        const executions = await base44.entities.WorkflowExecution.filter({ bot_id: botId }, '-created_date', limit);
        ...
        if (bot.trigger?.type === 'webhook' || bot.trigger?.type === 'api_endpoint') {
            webhookExecutions = await base44.entities.WebhookExecution.filter({ bot_id: botId }, '-created_date', 10);
        }

...
Deno.serve(async (req) => {
    try {
        ...
        if (bot.trigger?.type !== 'api_endpoint' && bot.trigger?.type !== 'webhook') {
            return Response.json({ error: 'This bot is not configured for webhook triggers' }, { status: 403 });
        }
        ...
        if (bot.trigger?.config?.payloadSchema) {
            ...
            if (!validationResult.valid) {
                return Response.json({ error: 'Invalid payload format', details: validationResult.errors }, { status: 400 });
            }
        }
